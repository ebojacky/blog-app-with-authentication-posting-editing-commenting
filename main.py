...
from forms import CreatePostForm, CreateUserForm, LoginForm, ContactForm, CommentForm
...
def admin_only(func):
    ...
    def wrapper_func(*args, **kwargs):
        if current_user.id != 1:
            abort(403, description=" You are not authorised to perform this action.")
...
def register():
    ...
    if register_form.validate_on_submit():
        ...
        user_in_db = User.query.filter_by(email=register_form.email.data).first()
...
def login():
    ...
    if login_form.validate_on_submit():
        user_logging_in = User.query.filter_by(email=login_form.email.data).first()

        if user_logging_in is not None:
            if check_password_hash(user_logging_in.password, login_form.password.data):
